...
export default (props: MediaGalleryProps): JSX.Element => {
  ...
  const getMediaGalleryTilesForParticipants = (
    ...
    const remoteParticipantsMediaGalleryItems = participants.map((participant) => (
      <div key={`${getId(participant.identifier)}-tile`} className={mediaGalleryStyle}>
        <RemoteStreamMedia
          key={getId(participant.identifier)}
          stream={participant.videoStreams[0]}
          isParticipantStreamSelected={
            props.dominantParticipants.filter((p) => p.participantId === getId(participant.identifier)).length > 0
          }
          label={participant.displayName ?? getId(participant.identifier)}
        />
      </div>
    ));
  ...
  const getSubstageMediaGalleryTilesForParticipants = (participants: RemoteParticipant[]): JSX.Element[] => {
    const remoteParticipantsMediaGalleryItems = participants.map((participant) => (
      <div key={`${getId(participant.identifier)}-tile`} className={substageMediaGalleryStyle}>
        <RemoteStreamMedia
          key={getId(participant.identifier)}
          stream={participant.videoStreams[0]}
  ...
  const participantsToLayout = props.remoteParticipants.sort((a, b) => {
    const isParticipantADominant =
      props.dominantParticipants.filter((p) => p.participantId === getId(a.identifier)).length > 0;
    const isParticiantBDominant =
      props.dominantParticipants.filter((p) => p.participantId === getId(b.identifier)).length > 0;
