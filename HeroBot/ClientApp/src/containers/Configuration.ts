...
import CallConfigurationScreen, { CallConfigurationScreenProps, TokenResponse } from '../components/Configuration';
...
import { initCallClient, joinGroup, registerToCallAgent, updateDevices } from '../core/callSideEffects';
...
import { getTokenForUser, getRefreshedTokenForUser } from '../utils/utils';
import { AzureCommunicationTokenCredential } from '@azure/communication-common';
import { setUserId } from '../core/actions/sdk';

const mapStateToProps = (state: State, props: CallConfigurationScreenProps) => ({
  ...
  getToken: async (): Promise<TokenResponse> => {
    const tokenResponse: CommunicationUserToken = await getTokenForUser();
    const userToken = tokenResponse.token;
    const userId = tokenResponse.user.communicationUserId;

    const tokenCredential = new AzureCommunicationTokenCredential({
      tokenRefresher: (): Promise<string> => {
        return getRefreshedTokenForUser(userId);
      },
...
const mapDispatchToProps = (dispatch: any, props: CallConfigurationScreenProps) => ({
...
export default connector(CallConfigurationScreen);
